...
and !$array['styles']['gradient']['rendered'] ) {
	...
	$sty['css_important'] = $this->tvr_css3_imp($section_name, $css_selector_slug, $property_group_name, 'gradient_c', $con, $mq_key);
...
!$array['styles']['radius']['rendered'] ) {
	...
	// !important is a bit different for css3 - only one "i" per line - do another check
	$sty['css_important'] = $this->tvr_css3_imp($section_name, $css_selector_slug, $property_group_name, 'border_bottom_left_radius', $con, $mq_key);
...
!$array['styles']['box_shadow']['rendered']) {
	...
	// !important is a bit different for css3 - only one "i" per line - do another check
	$sty['css_important'] = $this->tvr_css3_imp($section_name, $css_selector_slug, $property_group_name, 'box_shadow_inset', $con, $mq_key);
	...
	if (!empty($property_group_array['box_shadow_spread'])) {
		$box_shadow_spread = $property_group_array['box_shadow_spread'];
	}
	else {
		$box_shadow_spread = 0;
	}
	$box_shadow_spread.= $this->check_unit($property_group_name, 'box_shadow_spread', $box_shadow_spread);
	// shadow color
...
!$array['styles']['text_shadow']['rendered']) {
	...
	// !important is a bit different for css3 - only one "i" per line - do another check
	$sty['css_important'] = $this->tvr_css3_imp($section_name, $css_selector_slug, $property_group_name, 'text_shadow_color', $con, $mq_key);
