...
empty($this->propertyoptions[$property_group_name][$property]['hide imp'])) {
	$important_toggle.= '<span class="important-toggle input-icon-toggle '.$class.' imp-'.$con.'" data-input-type="important"
		title="' . esc_attr__('Add/remove !important declaration', 'microthemer') . '" rel="'.$rel.'">i</span>';
} else {
...
$html.= '<div id="opts-'.$section_name.'-'.$css_selector.'-'.$property_group_name.'-'.$property. $mq_extr_class. '"
 class="property-tag field-wrap tvr-clearfix '.$man_class . ' '.$field_class . ' field-'.$property_group_name.'-'.$property.'">
	...
	// adjust for lang
	$prop_label = $this->propertyoptions[$property_group_name][$property]['label'];
	if ($this->preferences['tooltip_en_prop']){
		$prop_label.= ' / '.str_replace('_', '-', $property).'';
	}

	$html.= '<span class="option-label"
	data-simple-tt="'.$prop_label.'"
	title="'.$prop_label.'">'
	...
	$html.= '
			...
			name="tvr_mcth'.$mq_stem.'['.$section_name.']['.$css_selector.'][styles]['.$property_group_name.']['. $property.$input_name_adj.']" value="'.$value.'" />
	...
	<span class="comp-mixed-table">
		<h5 title="' . esc_attr__('Hover over the rows to highlight the individual elements on the page', 'microthemer') . '">' . esc_html__('Page Analysis', 'microthemer') . '</h5>
		<table>
			<thead>
			<tr>
				<th><i>n</i></th>
				<th>' . esc_html__('Value', 'microthemer') . '</th>
			</tr>
			</thead>
			<tbody></tbody>
		</table>
