...
// if already defined, tell user to disable microloader
	...
	add_action(
		...
		create_function(
			'',
			'echo \'<div id="message" class="error"><p><strong>'.esc_html__('Microthemer requires that you deactivate Microloader. Please do so.', 'microthemer').'</strong></p></div>\';'
		)
	);
	...
	if (!class_exists('tvr_microthemer_admin')) {
		...
		class tvr_microthemer_admin {
			...
			*/
			function __construct(){
				...
				if ( is_admin() and in_array($page, $this->all_pages) ) {
					...
					$this->custom_code = array(
						'hand_coded_css' => esc_html__('All Browsers', 'microthemer'),
						'ie_css' => array(
					...
					$this->css_unit_sets = array(
						esc_html__('px (implicit)', 'microthemer'),
						esc_html__('em', 'microthemer'),
						esc_html__('px to em', 'microthemer'),
					);*/
					...
					$this->default_preferences = array(
						...
						"tooltip_en_prop" => $this->is_en() ? 0 : 1,
			...
			function update_my_props_array($new_css_units){
				...
				if ( ($this->memory_limit != 0) and ($this->memory_limit < 16 ) ) {
					add_action(
						...
						create_function(
							'',
							'echo \'<div id="message" class="error"><p><strong>' .
								esc_html__('Sorry, Microthemer requires a memory limit of 16MB or higher to run. Your memory limit is less than this. Deactivate Microthemer to remove this message.', 'microthemer') .
							'</strong></p></div>\';'
						)
					);
					return false;
			...
			function microthemer_dedicated_menu() {
				...
				add_submenu_page($this->microthemeruipage, __('Microthemer Preferences', 'microthemer'),
					__('Preferences', 'microthemer'), 'administrator', $this->preferencespage, array(&$this,'microthemer_preferences_page'));
			}
			...
			function getRevisions() {
				...
				<thead>
				<tr>
					<th>' . esc_html__('Revision', 'microthemer') . '</th>
					<th>' . esc_html__('Date & Time', 'microthemer') . '</th>
					<th>' . esc_html__('User Action', 'microthemer') . '</th>
				...
				foreach ($revs as $rev) {
					...
					<tr>
						...
						if ($total_rows == 1) {
							$rev_table.= esc_html__('(Oldest)', 'microthemer');
						}
						$rev_table.='</td>
						<td>'.$rev->datetime.'</td>
			...
			function saveUiOptions($theOptions){
				...
				if ($this->debug_save and !empty($this->preferences['theme_in_focus'])) {
					...
					$data.= "\n\n" . __('The existing options', 'microthemer') . "\n\n";
			...
			function update_default_css_units($pref_array){
				...
				$pref_array['my_props'] = $this->update_my_props_array($new_css_units);
				return $pref_array;
			}
				...
				if ( isset($_GET['page']) and $_GET['page'] == $this->microthemeruipage ) {
					...
					if ( isset($_GET['action']) and $_GET['action'] == 'tvr_microthemer_ui_load_styles') {
						...
						if ($this->debug_pulled_data){
							$debug_file = $this->micro_root_dir . $this->preferences['theme_in_focus'] . '/debug-pulled-data.txt';
							$write_file = fopen($debug_file, 'w');
							$data = '';
					...
					if ( isset($_POST['import_from_pack_name']) and !empty($_POST['import_from_pack_name']) ) {
						...
						// update the revisions DB field
						$user_action = wp_kses(
							sprintf( esc_html__('Import (%1$s) from %2$s', 'microthemer'),
								$context,
					...
					elseif( isset($_GET['action']) and $_GET['action'] == 'tvr_ui_reset'){
						...
						if ($this->resetUiOptions()) {
							...
							$this->log(
								esc_html__('Folders were reset', 'microthemer'),
					...
					elseif(isset($_GET['action']) and $_GET['action'] == 'tvr_clear_styles'){
						...
						if ($this->clearUiOptions()) {
							$this->update_active_styles('customised');
							$this->log(
								esc_html__('Styles were cleared', 'microthemer'),
					...
					if(isset($_GET['action']) and $_GET['action'] == 'restore_rev'){
						...
						if ($this->restoreRevision($rev_key)) {
							$this->log(
								esc_html__('Settings successfully restored', 'microthemer'),
								'<p>' . esc_html__('Your settings were successfully restored from a previous save.', 'microthemer') . '</p>',
								'notice'
							);
							$this->update_active_styles('customised');
							// update the revisions DB field
							if (!$this->updateRevisions($this->options, __('Previous Settings Restored', 'microthemer'))) {
								$this->log('','','error', 'revisions');
							}
						}
						else {
			...
			function validate_reminder() {
				if (!$this->preferences['buyer_validated'] and TVR_MICRO_VARIANT == 'themer') {
					...
					<p><b><?php esc_html_e('IMPORTANT - Free Trial Mode is Active', 'microthemer'); ?></b><br /> <br />
						...
						<br />
						<?php esc_html_e('The Free Trial limits you to editing or creating 9 Selectors.', 'microthemer'); ?>
					</p>
					<p><?php echo wp_kses(
			...
			***/
			...
			function display_left_menu_icons() {
				...
				$html = '
					...
					<div class="save-interface v-left-button" title="' . esc_attr__('Manually save UI settings (Ctrl+S)', 'microthemer') . '"></div>
					...
					<div class="code-tools v-left-button tvr-popright-wrap popup-show">
						...
						<div class="tvr-popright">
							<div class="popright-sub">
								<div class="display-css-code v-left-button show-dialog popup-show" rel="display-css-code" title="' . esc_attr__('View the CSS code Microthemer generates', 'microthemer') . '"></div>
							</div>
						</div>
					</div>
					...
					<div class="program-docs v-left-button show-dialog new-icon-group popup-show" rel="program-docs"
					...
					title="View Microthemer\'s integration options">
						<div class="tvr-popright integration-right">
							<div class="popright-sub integration-sub">
								...
								title="' . esc_attr__("Enable/Disable WPTouch mode", 'microthemer') . '">WPTouch</div>
								<div id="toggle-beaver" class="v-left-button toggle-beaver"
								title="' . esc_attr__("Enable/Disable Builder Beaver mode", 'microthemer') . '">Builder Beaver</div>
							</div>
						</div>
					</div>
				 */
			...
			function menu_section_html($section_name, $array) {
				...
				<li id="<?php echo 'strk-'.$section_name; ?>" class="section-tag strk strk-sec">
					...
					<input type='hidden' class='view-state-input section-tracker'
						   name='tvr_mcth[non_section][view_state][this]' value='0' />
			...
			function menu_selector_html($section_name, $css_selector, $array, $sel_loop_count) {
				...
				<li id="<?php echo 'strk-'.$section_name.'-'.$css_selector; ?>" class="selector-tag strk strk-sel <?php echo $sel_class; ?>">
					...
					<div class="sel-row item-row">
						<span class="tvr-icon selector-icon sortable-icon" title="<?php esc_attr_e("Reorder or move selector into a different folder", 'microthemer'); ?>"></span>
							...
							foreach ($this->polyfills as $poly){
								if ($this->preferences[$poly.'_by_default'] != 1){
									...
									if (!empty($this->options[$section_name][$css_selector][$poly])) {
										$input = '<input class="'.$poly.'-tracker" type="hidden"
					name="tvr_mcth['.$section_name.']['.$css_selector.']['.$poly.']" value="1" />';
										$state = 'on';
										$class = 'active';
									} else {
										$input = '';
									...
									$extra_explain = '';
									if ($poly == 'pie'){
										$extra_explain = __('Note: also adds a "position" value of "relative" unless you explicitly set a value for position (pie needs the target element or parent element to have a non-static position value in order to work).', 'microthemer');
									}
									echo $input;
									?>
									<span class="<?php echo $poly; ?>-toggle tvr-icon input-icon-toggle <?php echo $class; ?>"
										data-input-type="<?php echo $poly; ?>"
										title='<?php printf(esc_attr__('Click to turn the %1$s polyfill on or off for this selector. %2$s', 'microthemer'), $poly, $extra_explain); ?>'
...
";
					...
					if ( is_array($array) ) {
						foreach ( $array as $css_selector => $array) {
							$empty = true;
							// check if valid selector and array contains property values, if not, skip
							if ($css_selector == 'this' or empty($array['styles'])) {
								continue;
							}
							...
							if (!empty($array['styles']) and !is_array($array['styles'])) {
								...
								$this->log(
									esc_html__('Malformed error', 'microthemer'),
									'<p>'.$display_section_name. ' &raquo; '.$css_sel_name.'</p>
									<p>' .
									sprintf(
										esc_html__('To fix this issue, please delete the malformed \'%1$s\' selector (which may not be displaying a name) in the \'%2$s\' folder', 'microthemer'),
										$css_sel_name, $display_section_name
									) . '</p>' .
									esc_html__('Array is: ', 'microthemer') . $array['styles']
								);
							...
							if ($con == 'mq') {
								$array['label'] = $this->options[$section_name][$css_selector_slug]['label'];
							}
							...
							$css_selector = $this->unescape_cus_quotes($css_selector);
										...
										if ( is_array( $property_group_array ) ) {
											...
											foreach ($property_group_array as $property => $value) {
												...
												if (empty($this->options['non_section']['important'])) {
													...
													if ($con == 'mq') {
														$important_val = $this->options['non_section']['important']['m_query'][$mq_key][$section_name][$css_selector_slug][$property_group_name][$property];
													} else {
														$important_val = $this->options['non_section']['important'][$section_name][$css_selector_slug][$property_group_name][$property];
													}
												}
...
";
									...
									if ( empty($array['styles']['position']['position']) ) {
										$sty['data'].= $tab."	position: relative; /* " .
											esc_attr_x('Because CSS3 PIE is enabled. It requires this to work.', 'CSS comment', 'microthemer') . " */
";
									}
								}
...
";
								...
								if ($cusStyles and is_array($cusStyles)) {
									$sty['data'].= "	/* " . esc_attr_x('Selector Custom CSS', 'CSS comment', 'microthemer') . " */
";
...
";
			...
			function update_active_styles($activated_from, $context = '') {
				...
				if ( is_writable($act_styles) ) {
					...
					$poly_path = substr(getenv("SCRIPT_NAME"), 0, -19) . str_replace($this->site_url, '', $this->micro_root_url);
					$sty['pie'] = $poly_path . 'PIE.php';
					//$sty['boxsizing'] = $poly_path . 'boxsizing.php';
					// css3 properties
					$sty['css3'] = array(
						'gradient_a',
						'gradient_b',
						...
						$sty['data'].= '
/* =' . esc_attr_x('Hand Coded CSS', 'CSS comment', 'microthemer') . '
-------------------------------------------------------------- */
'.$this->options['non_section']['hand_coded_css'] . '
';
						}
						...
						if (!fwrite($write_file, $sty['data'])) {
							$this->log(
								...
								'<p>' . sprintf(esc_html__('Writing to %s failed.', 'microthemer'), $this->root_rel($act_styles)) . $this->permissionshelp.'</p>'
							);
						}
						fclose($write_file);
					}
						...
						if (!empty($trim_val)) {
							...
							if (!file_exists($stylesheet)) {
								if (!$write_file = fopen($stylesheet, 'w')) {
									$this->log(
										esc_html__('Create IE stylesheet error', 'microthemer'),
							...
							if ( is_writable($stylesheet) ) {
								...
								// if write is unsuccessful for some reason
								if (!fwrite($write_file, $val)) {
									$this->log(
										esc_html__('Write IE stylesheet error', 'microthemer'),
										'<p>' . sprintf(esc_html__('Writing to %s failed.', 'microthemer'), $this->root_rel($stylesheet))
										. $this->permissionshelp . '</p>'
									);
								}
								fclose($write_file);
				...
				if ($context != 'create-blank') {
					if ( is_writable($json_file) ) {
						...
						if ( empty($this->serialised_post['export_sections']['hand_coded_css'])) {
							...
							echo esc_html_x('unset: hand-coded', 'Verb', 'microthemer') . '<br />';
						} else {
							echo wp_kses(__('<b>don\'t</b> unset: hand-coded', 'microthemer'), array( 'b' => array())) . '<br />';
						}
						// ie too
						foreach ($this->preferences['ie_css'] as $key => $value) {
							if ( empty($this->serialised_post['export_sections']['ie_css'][$key])) {
								...
								echo esc_html_x('unset: ', 'Verb', 'microthemer') . $key. '<br />';
							} else {
								echo wp_kses(__('<b>don\'t</b> unset: ', 'microthemer'), array( 'b' => array())) . $key. '<br />';
							}
						}
			...
			function load_json_file($json_file, $theme_name, $context = '') {
				...
				if (file_exists($json_file)) {
					...
					if (!$json_error) {
						...
						// merge the arrays if merge
						if ($context == 'Merge') {
							$filtered_json = $this->merge($this->options, $filtered_json);
						}

						// compare media queries in import to existing
						...
						if ($mq_analysis['new']) {
							...
							$this->log(
								...
								'<p>' . esc_html__('The design pack you imported contained media queries that are different from the ones used in your current setup. In order for the design pack settings to function correctly, these additional media queries have been imported into your workspace.',
								'microthemer') . '</p>
								<p>' . wp_kses(
									sprintf(
										__('Please <span %s>review (and possibly rename) the imported media queries</span>. Note: imported queries are marked with "(imp)", which you can remove from the label name once you\'ve reviewed them.', 'microthemer'),
						...
						if (
							//$context != 'Merge' // i'm not sure if this line is needed
							$mq_analysis['replacements_needed']
							or $mq_analysis['new']) {
							$this->savePreferences($pref_array);
						}
					}
			...
			function merge($orig_settings, $new_settings) {
				...
				if ($this->debug_merge) {
					...
					$write_file = fopen($debug_file, 'w');
					$data = '';
					$data.= "\n\n" . __('The current options', 'microthemer') . "\n\n";
				...
				if (is_array($new_settings)) {
					...
					if ($this->debug_merge) {
						$data .= "\n\n" . esc_html__('The to merge options', 'microthemer') . "\n\n";
						...
						if ($key == 'hand_coded_css'){
							...
							if (!empty($new_code)) {
								$merged_settings['non_section'][$key] =
									...
									. "\n\n/* " . esc_html_x('CSS from design pack imported with \'Merge\'', 'CSS comment', 'microthemer') . " */\n"
				...
				if (!empty($student_mqs) and is_array($student_mqs)) {
					foreach ($student_mqs as $student_key => $student_array){
						...
						if ( !$replacement_key ) {
							...
							$mq_analysis['new'][$new_key]['label'] = $student_array['query']. esc_html_x(' (imp)', '(imported media query)', 'microthemer');
			...
			*/
			...
			function add_js() {
				if ( is_user_logged_in() and TVR_MICRO_VARIANT == 'themer') {
					...
					// load js strings for translation
					$js_i18n_overlay = $js_i18n_main = array();
					include_once $this->thisplugindir . 'includes/js-i18n.inc.php';
					$js_i18n_overlay = array_merge($js_i18n_overlay, $js_i18n_main);
